...
- `src/components/` — pantallas y formularios:
  ...
  - `Comprobantes` (`Comprobantes` en el menú): lista la copia congelada de cada comprobante de **salida** impreso (tabla `comprobantes`); permite ver el detalle y **reimprimir**. `ComprobanteSalida` — el comprobante térmico en sí (portal + hook `usarImpresionComprobante`), compartido por el carrito de salida y la reimpresión.
  - Numeración de salidas (`calcularNumeros`, `proximoNumero`, `etiquetaComprobante` del store): **solo las salidas** generan `Comprobante` (copia de lo que se imprime), con número correlativo `S-1`, uno por registro (las líneas de un carrito comparten número), reinicia cada año (`periodo`). Las entradas NO se numeran. El N° de la columna "N° de Salida" de los historiales y el del comprobante impreso salen de esa tabla; es fijo aunque después se edite o borre un movimiento.
- `supabase/migration-fase5.sql` — crea la tabla `comprobantes`.
