...
- `src/components/` — pantallas y formularios:
  ...
  - `Comprobantes` (`Comprobantes` en el menú): lista la copia congelada de cada comprobante de salida (tabla `comprobantes`); permite ver el detalle y **reimprimir**. `ComprobanteSalida` — el comprobante térmico en sí (portal + hook `usarImpresionComprobante`), compartido por el carrito de salida y la reimpresión.
  - Numeración automática (`calcularNumeros`, `proximoNumero`, `etiquetaComprobante` del store): al registrar una entrada o salida se guarda un `Comprobante` con número correlativo por tipo (`E-1` / `S-1`), uno por registro (las líneas de un carrito comparten número), reinicia cada año (`periodo`). El N° de la columna de los historiales y el del comprobante impreso salen de esa tabla; es fijo aunque después se edite o borre un movimiento.
