  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <KPICard label="Entradas" value={totalEntradas} sub="registros" accent="text-leaf-600" />
        <KPICard label="Salidas" value={totalSalidas} sub="registros" accent="text-brand-600" />
        <KPICard label="Hoy" value={todayMoves} sub={`movimientos (${today.split("-").reverse().join("/")})`} />
      </div>

      {/* Productos por categoría — siempre visible */}
      <div className="bg-white border border-stone-200 rounded-xl shadow-xs overflow-hidden">
        <div className="px-5 py-3.5 border-b border-stone-100">
          <h2 className="text-sm font-bold text-stone-800 uppercase tracking-wider">Productos por categoría</h2>
        </div>
        {categoryRows.length === 0 ? (
          <p className="px-5 py-6 text-sm text-stone-400 text-center">No hay categorías registradas.</p>
        ) : (
          <div className="divide-y divide-stone-50">
            {categoryRows.map(({ name, count, units }) => (
              <div key={name} className="flex items-center justify-between px-5 py-3">
                ...
                <span className={`text-sm font-semibold ${count === 0 ? "text-stone-300" : "text-stone-700"}`}>
      ...
      {/* Punto de reorden — productos por debajo de su stock mínimo */}
      {reorderItems.length > 0 && (
        <div className="border border-amber-200 bg-amber-50/60 rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-amber-200 flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-amber-800">Punto de reorden</h2>
            <span className="ml-auto text-xs bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-full font-semibold">
              {reorderItems.length}
            </span>
          </div>
          <div className="divide-y divide-amber-100/70">
            {reorderItems.map((i) => (
              <div key={i.codigo} className="flex items-center justify-between px-5 py-2.5 text-sm">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-mono text-xs text-amber-800 bg-amber-100/80 px-1.5 py-0.5 rounded">{i.codigo}</span>
                  <span className="text-stone-700 truncate">{i.descripcion}</span>
                </div>
                <span className="text-xs text-stone-600 flex-shrink-0">
                  <strong className="text-amber-700 font-semibold">{i.cantidadDisponible}</strong>
                  {i.unidadMedida ? ` ${i.unidadMedida}` : ""} · mín. {i.stockMinimo}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className={`border rounded-xl overflow-hidden ${noStockItems.length > 0 ? "bg-brand-50 border-brand-200" : "bg-white border-stone-200"}`}>
        <div className={`px-5 py-3.5 border-b flex items-center gap-2 ${noStockItems.length > 0 ? "border-brand-200" : "border-stone-100"}`}>
          <svg className={`w-4 h-4 ${noStockItems.length > 0 ? "text-brand-600" : "text-stone-300"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
          </svg>
          <h2 className={`text-sm font-bold uppercase tracking-wider ${noStockItems.length > 0 ? "text-brand-700" : "text-stone-800"}`}>
            Productos sin stock
          </h2>
          {noStockItems.length > 0 && (
            <span className="ml-auto text-xs bg-brand-100 text-brand-700 border border-brand-200 px-2 py-0.5 rounded-full font-semibold">
              {noStockItems.length}
            </span>
          )}
        </div>
