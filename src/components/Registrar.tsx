            ...
            <tbody className="divide-y divide-stone-50">
              {[...movements].reverse().slice(0, 8).map((m) => (
                <tr key={m.id} className="hover:bg-stone-50/60 transition-colors">
                  <td className="px-4 py-2.5 font-mono text-xs text-stone-500">{m.codigo}</td>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[11px] text-stone-400 font-normal">• {m.area}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${m.tipo === "Entrada" ? "bg-leaf-50 text-leaf-700" : "bg-brand-50 text-brand-600"}`}>
                      {m.tipo}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-right font-mono text-stone-700">{m.cantidad}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-stone-700">S/ {m.valor.toFixed(2)}</td>
                  <td className="px-4 py-2.5 text-stone-500">{m.fecha.split("-").reverse().join("/")}</td>
                  <td className="px-4 py-2.5 text-stone-600">{m.responsable}</td>
